...
def distance(W, vocab, ivocab, input_term1, input_term2):
    if input_term1 not in vocab and input_term2 not in vocab:
        f2.write('%s%s%s\%s\n' % (input_term1.ljust(20), input_term2.ljust(20), input_term1, input_term2))
    elif input_term1 not in vocab:
        f2.write('%s%s%s\n' % (input_term1.ljust(20), input_term2.ljust(20), input_term1))
    elif input_term2 not in vocab:
        f2.write('%s%s%s\n' % (input_term1.ljust(20), input_term2.ljust(20), input_term2))
    else:
...
if __name__ == "__main__":
    ...
    rootdir = '/data/sam676/TrialThree/Trial3_allPairsCOPY/'
    for subdir, dirs, files in os.walk(rootdir):
        for file in files:
            f = open(subdir + '/' + file, 'r')
            f2 = open('/data/sam676/TrialThree/Trial3_CosineDist_Scores/' + file, 'w')
            for line in f:
                array = []
