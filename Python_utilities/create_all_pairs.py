...
# To generate single files of labels per stimulus ID using the optional --ID_label_file flag,
# give a single file with all IDs and labels listed in comma-separated pairs, one per line.
...
# Seperate a list of <ID, label> pairs into one list of labels per ID, using ID to identify the file.
def split_ID_labels():
    with open(args.ID_label_file, 'r') as f:
        line = f.readline()
        ID_next, label = line.rstrip().split(',')
        ID_next = ID_next.zfill(ZERO_PAD)
        while True:
            ID_curr = ID_next
            # print(ID_curr, ID_next, label)
            ...
            with open(out_file, 'w') as o:
                while (ID_next == ID_curr):
                    if label != "NA":
                        o.write("{}\n".format(label))
                        # print("wrote {} to file for slide {}".format(label, ID_next))
                    line = f.readline()
                    if not line:
                        return
                    ID_next, label = line.rstrip().split(',')
                    ID_next = ID_next.zfill(2)
